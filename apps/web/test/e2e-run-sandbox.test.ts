...
describe("web run sandbox flow", () => {
	...
	it("renders WILL-RUN truth, approve wiring, and durable files after workspace updates", async () => {
		...
		const fetchImpl = vi.fn<typeof fetch>(async (input, init) => {
			...
			if (url === `/runs/${runId}/skills/preview` && init?.method === "POST") {
				return new Response(
					JSON.stringify({
						skillName: "policy-qa",
						description: "Policy checks with citations",
		...
		await vi.waitFor(() => {
			expect(
				root.querySelector("[data-run-skill-list]")?.textContent,
			).toContain("policy-qa");
		});
		expect(
			root.querySelector("[data-run-skill-list]")?.textContent,
		).not.toContain("hidden-workflow");
		const skillOptions = Array.from(
			root.querySelectorAll<HTMLOptionElement>(
				"[data-run-skill-options] option",
			),
