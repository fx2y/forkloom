...
describe("spec08 sql helpers", () => {
	it("reports violations when req/task closure has gaps", () => {
		...
		expect(formatSpec08ChecklistSummary(report)).toBe(
			"req_full_cover_miss=1, task_done=10, task_total=15, task_all_done=0, required_missing=1, required_not_done=1",
		);
	});
	...
	it("stays green when closure latch is fully satisfied", () => {
		...
		expect(formatSpec08ChecklistSummary(report)).toBe(
			"req_full_cover_miss=0, task_done=15, task_total=15, task_all_done=1, required_missing=0, required_not_done=0",
		);
	});
});
